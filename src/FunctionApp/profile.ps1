...
if (Test-Path $modulesPath) {
    ...
    # Import the main AADExporter module
    $mainModulePath = Join-Path $resolvedModulesPath "AADExporter.psm1"
    if (Test-Path $mainModulePath) {
        try {
            Write-Information "Importing main module: AADExporter.psm1"
            Import-Module $mainModulePath -Force -Verbose:$false
            Write-Information "Successfully imported AADExporter module"
        }
        catch {
            Write-Error "Failed to import AADExporter module: $($_.Exception.Message)"
            throw
        }
    }
    else {
        Write-Error "AADExporter.psm1 not found at: $mainModulePath"
    ...
    $additionalModules = Get-ChildItem -Path $resolvedModulesPath -Filter *.psm1 -Recurse | 
        Where-Object { $_.Name -ne "AADExporter.psm1" }
...
$requiredEnvVars = @('CLIENTID', 'EVENTHUBNAMESPACE', 'EVENTHUBNAME')
$presentVars = @()
$missingVars = @()
...
$criticalFunctions = @('Invoke-AADDataExport', 'Get-AzureADToken', 'Send-EventsToEventHub')
$availableFunctions = @()
$missingFunctions = @()
